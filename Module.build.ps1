...
task InstallDependencies {
    Install-Module -Scope CurrentUser -force -Name Pester -RequiredVersion 4.10.1
    Install-Module -Scope CurrentUser -force -Name PlatyPS
    Install-Module -Scope CurrentUser -force -Name PSScriptAnalyzer
}
...
task Analyze {
    $scriptAnalyzerParams = @{
        ...
        Recurse = $true
        Verbose = $false
        ExcludeRule = @('PSUseDeclaredVarsMoreThanAssignments','PSAvoidGlobalVars','PSReviewUnusedParameter')
    }
...
task unittests {
    $report = Invoke-Pester .\tests\unit -PassThru
    if ($failed = $report.testresult.where{-not $_.Passed}) {
        $failed|format-list Describe,Context,Name,Stacktrace
        throw "One or more Pester tests were failing."
    }
}

task clean {
    Get-ChildItem .output -Recurse|remove-item -force
}
...
#Default Task
#task . clean, makehelp, unittests, integrationtests, buildclean, testpublishing , testinstalledmodule, mergedevtomaster
...
task UpdateHelp {
    Import-Module .\PSVault.psd1 -Force
...
task UpdateJWTPackage {
    ...
    If ($LatestVersion -gt $ProductVersion) {
        ...
        Write-Output -InputObject ('Copy JWT binaries to PSVault Module')
        ...
        Write-Output -InputObject ('Copy Newtonsoft binaries to PSVault Module')
...
task CopyModuleFiles {
    ...
    if (-not (Test-Path .\output\PSVault)) {

        $null = New-Item -Path .\output\PSVault -ItemType Directory

    }

    Copy-Item -Path '.\en-US\' -Filter *.* -Recurse -Destination .\output\PSVault -Force
    Copy-Item -Path '.\lib\' -Filter *.* -Recurse -Destination .\output\PSVault -Force
    Copy-Item -Path '.\public\' -Filter *.* -Recurse -Destination .\output\PSVault -Force
    Copy-Item -Path '.\tests\' -Filter *.* -Recurse -Destination .\output\PSVault -Force
    ...
    Copy-Item -Path @(
        ...
        '.\PSVault.psd1'
        '.\PSVault.psm1'
    ) -Destination .\output\PSVault -Force        
}
#endregion
...
task UpdateManifest {
    ...
    Update-ModuleManifest -ModuleVersion $newVersion -Path .\PSVault.psd1 -ReleaseNotes $ReleaseNotes
}
#endregion
...
task PublishModule -If ($Configuration -eq 'Production') {
    Try {
        ...
        $params = @{
            Path        = ('{0}\Output\PSVault' -f $PSScriptRoot )
            NuGetApiKey = $env:psgallery
            ErrorAction = 'Stop'
        }
        Publish-Module @params
        Write-Output -InputObject ('PSVault PowerShell Module version published to the PowerShell Gallery')
    }
    Catch {
        throw $_
    }
